...
Configuration::Configuration(const BaseSysData& baseData, const Parameters& pars){
    ...
    if (pars.startingMode=="new") {
        ...
        assert(baseData.numNodes==curPosX.size());
        assert(baseData.numNodes==curPosY.size());
        inFile.close();
    }
    ...
    defGradInvTransXX.resize(baseData.numElements,1);
    defGradInvTransXY.resize(baseData.numElements,1);
    defGradInvTransYX.resize(baseData.numElements,1);
    defGradInvTransYY.resize(baseData.numElements,1);
    ...
    gradX.resize(baseData.numElements,baseData.numNodes);
    gradY.resize(baseData.numElements,baseData.numNodes);
...
void Configuration::update_post_processing_data(const BaseSysData& baseData, const Parameters& pars){
    ...
    P2 = ((CstressYY+CstressYX).dot((refArea.array()*areaRatio.array()).matrix()) + KWYY + (CstressXX+CstressXY).dot((refArea.array()*areaRatio.array()).matrix())+KWXX)/(2*LX*LY);
    S1 = 0.5*(Fv/lxNew-Fh/lyNew);
    S2 = 0.5*(KWWV-KWWH)/(LX*LY);
    S3 = ((CstressYY+CstressYX).dot((refArea.array()*areaRatio.array()).matrix()) + KWYY - (CstressXX+CstressXY).dot((refArea.array()*areaRatio.array()).matrix())-KWXX)/(2*LX*LY);
    S4 = ((CstressYY+CstressYX).dot((refArea.array()*areaRatio.array()).matrix())- (CstressXX+CstressXY).dot((refArea.array()*areaRatio.array()).matrix()))/(2*LX*LY);
...
void Configuration::dump_global_data(const Parameters& pars, char mode, char purpose){
    ...
    if (mode=='w'){
        ...
        dataFile << "TotalEnergy" << std::setw(20) << "extW" << std::setw(20) << "maxResidualF" << std::setw(20) << "pressure1" << std::setw(20) << "pressure2" <<  std::setw(20)<< "shearStress1"  << std::setw(20)<< "shearStress2"<< std::setw(20) << "shearStress3" << std::setw(20) << "shearStress4" << std::setw(20) << "boxArea" << std::setw(20) << "phi" << std::setw(20) << "e0" << std::setw(20) << "e1" << std::endl;
        dataFile.close();
    }else if (mode=='a'){
        std::ofstream dataFile;
        dataFile.open (pars.outputFolderName+fname,  std::ios_base::app);
        dataFile << std::setprecision(9)<<  totalEnergy << std::setw(20) <<  extW << std::setw(20) << maxR << std::setw(20) << P1 << std::setw(20)<< P2 <<  std::setw(20)<< S1 << std::setw(20)<< S2 << std::setw(20) << S3 << std::setw(20) << S4 << std::setw(20) << A << std::setw(20) << phi << std::setw(20) << e0 << std::setw(20) << e1 << std::setw(20) << std::endl;
        dataFile.close();
...
void Configuration::dump_per_node(const BaseSysData& baseData, const Parameters& pars, int& timeStep){
    ...
    myfile << "id" << std::setw(20)<< "x" << std::setw(20) << "y" << std::setw(20) << "fx" << std::setw(20) << "fy" << std::endl;
    
    for (int i=0; i<curPosX.size();i++ ) {
        myfile << i << std::setw(20)<< curPosX[i] << std::setw(20)<< curPosY[i] << std::setw(20)<< forceX[i] << std::setw(20)<< forceY[i] << std::endl;
    }
...
void Configuration::dump_per_ele(const BaseSysData& baseData, const Parameters& pars, int& timeStep){
    ...
    myfile << "timeStep" << "\t" << timeStep << std::endl;
    myfile << "wallsLRBT" << "\t" << leftPos << "\t" << rightPos << "\t" << botPos << "\t" << topPos << std::endl;
    myfile << "id" << std::setw(20) << "refArea" << std::setw(20)<< "areaRatio" << std::setw(20)<< "FXX" << std::setw(20) << "FXY" << std::setw(20) << "FYX" << std::setw(20)<<"FYY" <<std::setw(20)<<"PK1StressXX" <<std::setw(20)<<"PK1StressXY" <<std::setw(20)<<"PK1StressYX" <<std::setw(20)<<"PK1StressYY" <<std::setw(20)<< "CStressXX" <<std::setw(20)<<"CStressXY" <<std::setw(20)<<"CStressYX" <<std::setw(20)<<"CStressYY" <<std::setw(20)<<"swellingPressure" <<std::setw(20)<<"elasticEnergy" <<std::setw(20)<<"mixingEnergy" <<std::setw(20)<<"internalEnergy" <<std::endl;
    
    for (int i=0; i<refArea.size();i++ ) {
        myfile << i << std::setw(20) << refArea[i] << std::setw(20)<< areaRatio[i] << std::setw(20) << defGradXX[i] << std::setw(20)<< defGradXY[i] << std::setw(20)<< defGradYX[i] << std::setw(20)<<defGradYY[i] <<std::setw(20)<< PK1stressXX[i] <<std::setw(20)<< PK1stressXY[i] <<std::setw(20)<< PK1stressYX[i] << std::setw(20)<< PK1stressYY[i] <<std::setw(20)<< CstressXX[i] <<std::setw(20)<< CstressXY[i] <<std::setw(20)<< CstressYX[i] <<std::setw(20)<< CstressYY[i] <<std::setw(20)<< swellingPressurePerEle[i] <<std::setw(20)<< elasticEnergyPerEle[i] <<std::setw(20)<< mixingEnergyPerEle[i] <<std::setw(20)<< internalEnergyPerEle[i] <<std::endl;
    }
