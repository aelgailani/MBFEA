...
void Configuration::compute_surface_forces_1(const BaseSysData& baseData, const Parameters& pars, const int& timeStep)
{
    maxInterference = 0;
        ...
        for (int cellXid=0; cellXid<numXCells; cellXid++)
        {
            ...
            while (slaveNodeId>=0)
            {
                ...
                for (auto const& delta: neighborBinDelta)
                {
                    ...
                    int nCellFlatId = + baseData.numSurfaceNodes;
