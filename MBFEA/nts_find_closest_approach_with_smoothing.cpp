...
//        std::cout<< "x4  " << node2 << std::endl;
    ...
    else {///// in case s lies left to the segment then check the upstream curve. Notice here x2-->node2, x3-->node0, x4-->node1
        
        int node2 =  baseData.surfaceSegments[baseData.surfaceSegments[segment][3]][0]; //// baseData.surfaceSegments[segment][4] gives downstream segment ID
        double x2 = augmentedCurPosX[node2];
        double y2 = augmentedCurPosY[node2];
